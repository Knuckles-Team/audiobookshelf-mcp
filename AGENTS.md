...
- Architecture:
    - `audiobookshelf_mcp/api/`: Modular folder for target service client wrappers.
    - `audiobookshelf_mcp/mcp/`: Modular folder for action-routed dynamic MCP tool tags.
    - `audiobookshelf_mcp/mcp_server.py`: Main MCP server entry point and tool registration.
    - `audiobookshelf_mcp/agent_server.py`: Pydantic AI agent definition and logic.
...
- MCP Entry Point → `audiobookshelf_mcp/mcp_server.py`
- Agent Entry Point → `audiobookshelf_mcp/agent_server.py`
- Source Code → `audiobookshelf_mcp/`
- API client mixins → `audiobookshelf_mcp/api/`
- MCP tool modules → `audiobookshelf_mcp/mcp/`
- Tests → `tests/`
