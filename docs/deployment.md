...
`audiobookshelf-mcp` exposes its MCP server (console script `audiobookshelf-mcp`) four ways. Pick the
...
```bash
uvx --from audiobookshelf-mcp audiobookshelf-mcp --transport streamable-http --host 0.0.0.0 --port 8000
...
{
  "mcpServers": {
    "audiobookshelf-mcp": { "url": "http://localhost:8000/mcp" }
  }
}
```
  ...
  "mcpServers": {
    "audiobookshelf-mcp": {
      ...
      "args": [
        "run", "-i", "--rm",
        "-e", "TRANSPORT=stdio",
        "-e", "AUDIOBOOKSHELF_URL=https://service.example.com",
        "-e", "AUDIOBOOKSHELF_TOKEN=your_token",
        "knucklessg1/audiobookshelf-mcp:latest"
      ]
    }
  }
}
...
{
  "mcpServers": {
    "audiobookshelf-mcp": { "url": "http://audiobookshelf-mcp.arpa/mcp" }
  }
}
```

Caddy reverse-proxies `http://audiobookshelf-mcp.arpa` to the container's `:8000`
